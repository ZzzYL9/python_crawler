...
class DushiSpider(scrapy.Spider):
    name = 'dushi'
    allowed_domains = ['ranwen8.com']
    start_urls = ['https://www.ranwen8.com/book/118098/45222090.html']

    # def get_infos(self):
    conn = sqlite3.connect('novel_info.db')
    cursor = conn.cursor()
    cursor.execute('SELECT novel_name, novel_start FROM DUSHI')
    ...
    # print(novel_names)
    #
    # conn = sqlite3.connect('./novel_db/dushi.db')
    # cursor = conn.cursor()
